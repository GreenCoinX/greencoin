...
if [ $# -lt 1 ]; then
        ...
        echo "e.g. $0 ../../src"
        echo "Env vars GREENCOIND and GREENCOINCLI may be used to specify the exact binaries used"
        exit 1
fi

set -f

GREENCOIND=${GREENCOIND:-${1}/greencoind}
CLI=${GREENCOINCLI:-${1}/greencoin-cli}
...
$GREENCOIND -zapwallettxes=1 $B1ARGS &
B1PID=$!
$GREENCOIND -zapwallettxes=2 $B2ARGS &
B2PID=$!
